...
// Definicao de grafo utilizando matriz de adjacencia
typedef struct
{
...
typedef struct
{
  EstadoVertice estadoVertice;
  unsigned int descoberta;
  unsigned int final;
  unsigned int pai;
  unsigned int numeroDoVertice;
...
/*
 * Lista de 269 cores distintas
 * http://godsnotwheregodsnot.blogspot.com/2013/11/kmeans-color-quantization-seeding.html
 */
const static int GRAFO_H_TOTAL_DE_CORES = 269;
const static char GRAFO_H_LISTA_CORES[][8] = {"#000000", "#FFFF00", "#1CE6FF", "#FF34FF", "#FF4A46", "#008941",
...
void buscar_em_profundidade_visita(const GRAFO *grafo, unsigned int de, unsigned int *relogio_global, RSLT_BUSCA *resultado_busca);
