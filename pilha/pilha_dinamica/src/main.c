...
int main ()
{
  ...
  if (!pilhadescritor_inicializa(&p2))
  {
    return 1;
  }
  printf("Numero de elementos: %d\n", pilhadescritor_numelementos(&p2));

  if (pilhadescritor_estavazia(&p2))
  {
    printf("Pilha esta vazia\n");
  }
  else
  ...
  for (i = 0; i < 15; i++)
  {
    if (pilhadescritor_PUSH(&p2, (TIPO_DADO *) &i))
    {
      printf("Inserido %d\n", i);
      printf("Numero de elementos: %d\n", pilhadescritor_numelementos(&p2));
    }
  ...
  if (pilhadescritor_estacheia(&p2))
  {
    printf("Pilha esta cheia\n");
  }
  else
  ...
  while (pilhadescritor_POP(&p2, &c))
  {
    printf("%d\n", c);
  }

  printf("Destruindo a pilha\n");
  pilhadescritor_destroi(&p2);
